...
switch ($act) {
    case 'envCheck':
        ...
        foreach ($data as $key => $row){
            ...
            if($row["type"] == "php" ){
                $data[$key]["now"] = phpversion();
                $data[$key]["ok"] = version_compare(phpversion(), '7.2', '>=') && version_compare(phpversion(), '8.0', "<");
                $data[$key]["msg"] = $data[$key]["ok"]?"":"PHP版本不兼容";
            }
            
            if($row["type"] == "exec"){
                $data[$key]["now"] = function_exists('exec')?"可用":"";
                $data[$key]["ok"] = function_exists('exec');
                $data[$key]["msg"] = $data[$key]["ok"]?"":"被禁用";
            }
            ...
            if($row["type"] == "mysql" ){
                ...
                $data[$key]["ok"] = function_exists('exec');
                if(!$data[$key]["ok"]){
                    $data[$key]["msg"] = "请先解禁exec函数";
                }else{
                    $data[$key]["now"] = trim(explode(' ', exec('mysql -V'))[5], ',');
                    $data[$key]["ok"] = version_compare(trim(explode(' ', exec('mysql -V'))[5], ','), '5.0', '>');
                    $data[$key]["msg"] = $data[$key]["ok"]?"":"Mysql版本不兼容";
                }
            }
            
            if($row["type"] == "php_redis"){
                ob_start();
                phpinfo(INFO_MODULES);
                $phpinfo = ob_get_clean();
                $data[$key]["now"] = strpos($phpinfo, 'redis')?"已安装":"";
                $data[$key]["ok"] = strpos($phpinfo, 'redis');
                $data[$key]["msg"] = $data[$key]["ok"]?"":"未安装";
            }
            
            if($row["type"] == "shell_exec"){
                $data[$key]["now"] = function_exists('shell_exec')?"可用":"";
                $data[$key]["ok"] = function_exists('shell_exec');
                $data[$key]["msg"] = $data[$key]["ok"]?"":"被禁用";
            }
            
            if($row["type"] == "curl"){
                $data[$key]["now"] = extension_loaded('curl')?"已加载":"";
                $data[$key]["ok"] = extension_loaded('curl');
                $data[$key]["msg"] = $data[$key]["ok"]?"":"未加载";
            }
...
// 需要排除初始化链接mysql的
if ($act !== 'mysql_c' && $act !== 'mysql_is' && $act !== 'envCheck' ) {
